...
namespace PTMK_Test.Application.Implementation.Application.Queries
{
    ...
    public readonly record struct GetApplicationsReportQuery : IRequest<ApplicationsReportDto>;

    public sealed class GetApplicationsReportHandler(IDbContext dbContext)
        : IRequestHandler<GetApplicationsReportQuery, ApplicationsReportDto>
    {
        private readonly IDbContext _dbContext = dbContext;

        public async Task<ApplicationsReportDto> Handle(GetApplicationsReportQuery request, CancellationToken ct)
        { 
            ...
            {
                { nameof(ApplicationStatusType.New), summaryMetrics?.NewCount ?? 0 },
                { nameof(ApplicationStatusType.InProgress), summaryMetrics?.InProgressCount ?? 0 },
                { nameof(ApplicationStatusType.Completed), summaryMetrics?.CompletedCount ?? 0 }
            };

            int totalOverdue = summaryMetrics?.OverdueCount ?? 0;

            var completedApplicationsQuery = _dbContext.Applications.Where(completedSpec);
            ...
            return new ApplicationsReportDto(reportByStatus, totalOverdue, completedByExecutors);
        }
    }
}
